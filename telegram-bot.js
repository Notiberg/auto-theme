...
class TelegramBot {
    ...
    initTelegramWebApp() {
        if (window.Telegram && window.Telegram.WebApp) {
            ...
            // Получаем информацию о пользователе и chat_id
            this.user = this.tg.initDataUnsafe?.user;
            this.chatId = this.user?.id || null;
            
            // Дополнительно пытаемся получить chat_id из query_id
    ...
    async sendBookingConfirmation(bookingData) {
        if (!this.chatId) {
            console.log('Chat ID не найден - показываем обычное уведомление');
        ...
        try {
            ...
            if (response.ok && result.ok) {
                console.log('Уведомление успешно отправлено пользователю:', this.chatId);
...
ℹ️ Вы можете создать новую запись в любое время.`;
        ...
        try {
            ...
            if (response.ok && result.ok) {
                console.log('Уведомление об отмене отправлено пользователю:', this.chatId);
            } else {
                console.error('Ошибка отправки уведомления об отмене:', result);
            }
        } catch (error) {
